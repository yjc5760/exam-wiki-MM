...
2. 轉換為精簡陣列格式寫入 dashboard-data.js：
   [moduleId, primaryTopicId縮寫(去MM-前綴), secondaryTopicIds縮寫, designMethod, viz檔名前綴陣列, tags, pdf補充筆記檔名陣列]
...
【高頻考點 Top 10】
MM-U1-1 MM 梁彎矩強度分析與設計：X 題（主：X 題，副：X 題）
MM-U2-1 MM 剪力強度分析與設計：X 題
...
...
-----|------------|----------|--------|------|--------
1   | MM-U2-2 扭力 | MM-U2-1 | USD   | ★★★  | ✅ verified
...
...
- `study MM-U1-1`（子項層級深度複習，Cowork 直接執行）
...
③ 考題清單（互動篩選）
  ...
  - 【重要】點擊題號或標題時，必須以 `<a href="../index.html#md=raw/solutions/MM-XXXX-N/MM-XXXX-N.md&t=MM-XXXX-N" target="_blank">` 格式連結至 markdown 渲染器，確保公式能透過 KaTeX 呈現，且相對路徑的附圖（png）與補充檔（pdf）皆可正常載入。
...
**命名規則：**
- 單元層級：`study/study-MM-UN.html`（例：`study/study-MM-U1.html`）
- 子項層級：`study/study-MM-UN-n.html`（例：`study/study-MM-U1-1.html`）
