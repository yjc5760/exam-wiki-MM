...
| 指令 | 觸發語句 | 用途 |
|------|---------|------|
| [INGEST](#ingest) | `ingest MM-XXXX-N` | 將一道已驗證題目寫入 wiki |
...
| [STUDY](#study) | `study MM-UN` | 彙整某單元所有題目與重點 |
...
| [RELATED](#related) | `related MM-XXXX-N` | 找出與某題考點相似的其他題目 |
...
**觸發語句：** `ingest MM-2018-1`（Cowork 直接執行）

**前置檢查（強制）：**
```
1. 讀取 raw/json/question_index.json
2. 找到 moduleId = "MM-2018-1" 的條目
...
**執行步驟：**
```
1. 讀取 raw/solutions/MM-XXXX-N/MM-XXXX-N.md
2. 讀取 raw/json/question_index.json 中該題的完整條目
   → 取得 primaryTopicId、secondaryTopicIds、analysisMethod、tags、hasViz
3. 掃描 raw/solutions/MM-XXXX-N/ 下的所有附屬檔案：
   → *-fig-*.png、*-chart-*.png、*-eqn-*.png（靜態截圖）
   → *-hand-*.png（手寫補充）
   → *-beam-viz.html、*-mohr-viz.html 等（互動圖）
4. 建立或更新 wiki/problems/MM-XXXX-N.md
...
**錯誤更正流程：**
```
① 對 Cowork 說：「將 MM-XXXX-N 的 verificationStatus 改為 needs-review」
② 對 Cowork 說：在 raw/solutions/MM-XXXX-N/MM-XXXX-N.md 末尾補充更正說明
③ 人工重新驗算確認後：「將 MM-XXXX-N 的 verificationStatus 改回 verified」
④ 對 Cowork 說：ingest MM-XXXX-N
```
...
4. 生成 wiki/index.md（依 MM 命題大綱分類）
...
6.  Mohr's circle 圖缺口（MM-U1-3 應力轉換題目但無 mohr-viz.html）
...
14. failure-modes/ 缺口（四大類別頁面是否齊全：降伏/挫屈/斷裂/過大變形）
15. materials/ 缺口（三大主題頁面是否齊全：彈性塑性材料/脆性材料/疲勞）
16. 輸出待補清單，依優先順序排列
```
...
**執行步驟：**
```
1. 掃描 raw/solutions/ 下所有子資料夾（格式：MM-YYYY-N）
...
**觸發語句：** `add concept [概念名]`（例：`add concept 形心計算`）
...
```
1. 詢問概念的基本資訊：
   - concept_id（建議格式：全大寫-連字號，如 CENTROID-CALCULATION）
   - 中文名稱、英文名稱
   - 所屬單元（MM-UN-n）
   - 簡短定義（1-2句）
...
**觸發語句：** `add method [方法名]`（例：`add method Mohr's circle 應力轉換法`）
...
```
1. 詢問方法論基本資訊：
   - method_id（建議格式：全小寫-連字號，如 mohrs-circle）
   - 適用題型（MM-UN-n）
   - 核心公式、步驟摘要
...
2. 轉換為精簡陣列格式寫入 dashboard-data.js：
   [moduleId, primaryTopicId縮寫(去MM-前綴), secondaryTopicIds縮寫, analysisMethod, viz檔名前綴陣列, tags]
   - viz 前綴：掃描 raw/solutions/MM-XXXX-N/ 下 *-viz.html，
     取檔名中 moduleId 與 -viz.html 之間的字段（如 beam、mohr、section）
...
【高頻考點 Top 10】
MM-U2-2 梁桿件斷面應力計算：X 題（主：X 題，副：X 題）
MM-U3-2 梁桿件變位及內力分析：X 題
...
...
-----|----------------|----------|--------|------|--------
1   | MM-U2-2 梁斷面  | MM-U1-1  | 彈性分析 | ★★★ | ✅ verified
...
...
③ 考題清單（互動篩選）
  ...
  - 【重要】點擊題號或標題時，必須以 `<a href="../index.html#md=raw/solutions/MM-XXXX-N/MM-XXXX-N.md&t=MM-XXXX-N" target="_blank">` 格式連結至 markdown 渲染器，確保公式能透過 KaTeX 呈現，且相對路徑的附圖（png）與補充檔皆可正常載入。
...
### 子項層級（study MM-UN-n）頁面結構（七區塊）：
```
① 命題分析（KPI + 題型分類卡 + 年度堆疊長條圖 + 題型圓餅圖）
② 截面圖解（SVG 結構圖：梁型/斷面型/應力分布圖等）
③ 解題流程圖（SVG 決策樹）
④ 核心公式速查（KaTeX 分題型公式卡）
⑤ 考題清單（互動篩選，依題型分色）
⑥ 高頻陷阱（考古題歸納，依題型標色）
⑦ 互動計算器（輸入截面參數 → 即時計算並繪製應力/撓度圖）
```

**資料來源：** `raw/json/question_index.json`（從中統計各子項題數、年度分布、tags）

**命名規則：**
- 單元層級：`study/study-MM-UN.html`（例：`study/study-MM-U2.html`）
- 子項層級：`study/study-MM-UN-n.html`（例：`study/study-MM-U2-2.html`）

---

## FIND

**觸發語句：** `find [關鍵字]`（例：`find Mohr's circle`、`find 撓度積分`）
...
MM-XXXX-N：[題型摘要]（出現位置：標題/tags/解析內容）
...
```
...
**觸發語句：** `related MM-XXXX-N`（例：`related MM-2018-2`）

**用途：** 根據 primaryTopicId、secondaryTopicIds、tags 的重疊程度，找出與指定題目最相關的其他題目，方便集中練習同類題型。

**輸出格式：**
```
【與 MM-XXXX-N 相關的題目】（依相似度排序）

★★★ MM-YYYY-N：[共同考點] [共同標籤X個]
★★☆ MM-YYYY-N：...
★☆☆ MM-YYYY-N：...
```
...
MM-2018-1：T形截面彎曲正向應力與剪應力
MM-2018-2：圓桿扭轉應力與扭轉角
MM-2018-3：梁撓度（積分法）
MM-2018-4：Euler 挫屈臨界載重
...

共 X 題待驗算。驗算完成後說：「將 MM-XXXX-N 的 verificationStatus 改為 verified」
```
...
**範例：**
- 「哪些題目考到 Mohr's circle？」
- 「MM-U4 塑性分析共出了幾題？」
- 「2015 到 2020 年有哪些題目考挫屈？」
...
| 日期 | 變更 | 原因 |
|------|------|------|
| 2026-05-29 | 初版（RC 版本） | RC 科目建庫 |
| 2026-06-04 | 全面改寫：所有指令改由 Cowork 直接執行 | 兩層架構 |
| 2026-06-04 | 新增 10 個指令（備考分析 + 查詢快捷類） | 擴充功能 |
| 2026-06-29 | 全面改寫為 MM（材料力學）科目 | 科目轉換 |
